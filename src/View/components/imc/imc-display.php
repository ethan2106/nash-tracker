 ...
 * @requires Alpine.js - Variables: imc, imcInfo, imcMarkerPosition
 *
...
<div class="bg-blue-50/60 rounded-2xl shadow-lg p-8 flex flex-col items-center gap-4">
    ...
    <!-- ===== SCORE IMC ===== -->
    <span class="font-bold text-5xl text-blue-400 transition-colors duration-500"
          x-text="imc.toFixed(1)"
    ...
    <!-- ===== BADGE CATÉGORIE ===== -->
    <span class="px-4 py-1 rounded-full font-semibold transition-colors duration-500"
          :style="`background-color: ${imcInfo.color}20; color: ${imcInfo.color.replace('#', '')}cc;`"
          x-text="imcInfo.cat"
          :title="`Catégorie IMC: ${imcInfo.cat}`"></span>
    ...
    <div class="w-full mt-4">
        <div class="h-2.5 md:h-4 rounded-xl bg-gradient-to-r from-blue-300 via-green-300 via-yellow-300 via-orange-300 to-red-400 relative overflow-hidden"
            ...
            aria-valuemin="16"
            aria-valuemax="40"
            :aria-label="`Barre IMC : ${imc.toFixed(1)}`">
            <!-- Marqueur IMC dynamique -->
            <div id="imc-marker" class="absolute top-0 h-2.5 md:h-4 w-2 rounded transition-all duration-700"
                :style="`left: ${imcMarkerPosition}%; background-color: ${imcInfo.color}; box-shadow: 0 0 8px 2px ${imcInfo.color};`"
                :aria-label="`Marqueur IMC à ${imc.toFixed(1)}`"></div>
        </div>
    </div>
...
<div class="mt-3 text-center">
    <div class="text-sm font-medium text-gray-700 mb-1">Données utilisées pour le calcul</div>
    <div class="text-gray-600 bg-gray-50 rounded-lg px-3 py-2 inline-block"
         :title="'Taille et poids actuels utilisés dans la formule IMC = poids / (taille/100)²'"
         x-text="`Taille : ${taille} cm | Poids : ${poids} kg`"></div>
</div>

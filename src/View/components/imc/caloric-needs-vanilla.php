 ...
 *
 * @var array $data Données IMC depuis le contrôleur
 */

declare(strict_types=1);

// Valeurs par défaut pour l'affichage initial (seront mises à jour par JS)
$bmr = $data['bmr'] ?? 0;
$tdee = $data['tdee'] ?? 0;
$caloriesPerte = $data['calories_perte'] ?? 0;
$caloriesMaintien = $data['calories_maintien'] ?? 0;
$caloriesMasse = $data['calories_masse'] ?? 0;
?>
<!-- ============================================================
...
<div class="bg-white/70 backdrop-blur-xl rounded-3xl shadow-2xl border border-blue-100 p-8 flex flex-col gap-6 mt-12">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-gradient-to-br from-blue-50 to-blue-100 backdrop-blur-sm rounded-2xl shadow-lg p-6 flex flex-col items-center border border-blue-200 hover:shadow-xl transition-all duration-300 group">
            ...
            <span class="text-blue-600 font-bold text-3xl" id="bmr-value"
                  role="status" aria-label="Métabolisme de base" aria-live="polite"><?= round($bmr); ?> kcal/jour</span>
        ...
        <div class="bg-gradient-to-br from-green-50 to-emerald-100 backdrop-blur-sm rounded-2xl shadow-lg p-6 flex flex-col items-center border border-green-200 hover:shadow-xl transition-all duration-300 group relative">
            ...
            <div class="relative w-24 h-24 mb-3">
                <svg class="w-24 h-24 transform -rotate-90" viewBox="0 0 36 36">
                    ...
                    <path class="tdee-progress" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                          fill="none" stroke="#10b981" stroke-width="3"
                          stroke-dasharray="<?= round(($tdee / 3000) * 100); ?> 100"
            ...
            <span class="text-green-600 font-bold text-3xl" id="tdee-value"
                  role="status" aria-label="Dépense énergétique totale" aria-live="polite"><?= round($tdee); ?> kcal/jour</span>
    ...
    <div class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-white/80 rounded-xl shadow p-4 flex flex-col items-center">
            ...
            <span class="text-red-600 font-bold text-lg" id="perte-value"
                  role="status" aria-label="Calories pour perte de poids" aria-live="polite"><?= $caloriesPerte; ?> kcal/jour</span>
        </div>
        
        <!-- Maintien -->
        <div class="bg-white/80 rounded-xl shadow p-4 flex flex-col items-center">
            <span class="font-semibold text-gray-700">Maintien</span>
            <span class="text-yellow-600 font-bold text-lg" id="maintien-value"
                  role="status" aria-label="Calories pour maintien" aria-live="polite"><?= $caloriesMaintien; ?> kcal/jour</span>
        </div>
        
        <!-- Prise de masse -->
        <div class="bg-white/80 rounded-xl shadow p-4 flex flex-col items-center">
            <span class="font-semibold text-gray-700">Prise de masse</span>
            <span class="text-purple-600 font-bold text-lg" id="masse-value"
                  role="status" aria-label="Calories pour prise de masse" aria-live="polite"><?= $caloriesMasse; ?> kcal/jour</span>
        </div>
    </div>
</div>
